...
Zweck: Automatische Auswertung von MTF-Kurven, Kennfrequenzen (MTF50, MTF10) und Integralen (AUC) 
       mittels PyImageJ, Signalverarbeitung und Polynomialer Ausgleichsrechnung (3. Grad).
"""
...
if __name__ == "__main__":
    ...
    for folder_path in folder_paths:
        ...
        try:
            ...
            # SCHRITT 4: POLYNOMIALE AUSGLEICHSRECHNUNG & KENNZAHLEN
            if all_mtf_results:
                print("\n--------------------------------------------------")
                print(" BERECHNE KENNZAHLEN (POLYNOMIALER FIT 3. GRAD)")
                print("--------------------------------------------------")
                ...
                # Erster Durchgang: Nullstellen, Polynom-Koeffizienten und unnormierte Kurven bestimmen
                for single_mtf in all_mtf_results:
                    p_coeff = np.polyfit(x_freqs_orig, single_mtf, 3)
                    all_poly_coeffs.append(p_coeff)
                    p_func = np.poly1d(p_coeff)
                    
                    # Unnormierte Kurve auswerten
                    y_single_abs = np.clip(p_func(x_fit), 0, 1)
                    ...
                    # Errechnete Nullstelle (y=0) im erweiterten Suchbereich finden
                    y_s_search = p_func(x_search)
                    z_c = x_search[y_s_search <= 0]
                    xz_s = z_c[0] if len(z_c) > 0 else max(x_freqs_orig)
                    single_x_zeros.append(xz_s)

                mean_x_zero = np.mean(single_x_zeros)

                # Mittleres Polynom berechnen: y = a*x^3 + b*x^2 + c*x + d
                mean_poly_coeffs = np.mean(all_poly_coeffs, axis=0)
                a_coeff, b_coeff, c_coeff, d_coeff = mean_poly_coeffs
                
                # Formatierung des Polynom-Strings für Konsolen-Ausgabe und CSV-Export
                poly_str = f"y = {a_coeff:+.4f}*x^3 {b_coeff:+.4f}*x^2 {c_coeff:+.4f}*x {d_coeff:+.4f}"

                x_norm_start = x_min_real / mean_x_zero
                x_fit_norm_common = np.linspace(x_norm_start, 1.0, 500)

                # Zweiter Durchgang: Normierte Kurven auswerten
                single_curves_norm = []
                single_aucs_norm = []
                ...
                for single_mtf, xz_s in zip(all_mtf_results, single_x_zeros):
                    ...
                    y_single_norm = np.clip(p_func(x_fit_norm_single * xz_s), 0, 1)
                    
                    single_aucs_norm.append(abs(trap_func(y_single_norm, x_fit_norm_single)))

                    y_common_norm = np.clip(p_func(x_fit_norm_common * mean_x_zero), 0, 1)
                ...
                print("--------------------------------------------------")
                print(f"Mittleres Fit-Polynom (3. Grad): {poly_str}")
                ...
                print("--- UNNORMIERTES INTEGRAL (AUC Absolut) ---")
                ...
                pd.DataFrame([{
                    'Polynom_Formel': poly_str,
                    'Polynom_a_x3': a_coeff,
                    'Polynom_b_x2': b_coeff,
                    'Polynom_c_x': c_coeff,
                    'Polynom_d_const': d_coeff,
                ...
                # Hochauflösende MTF-Diagramme (.png) mit Standardabweichung (Shading) generieren
                ...
                ax1.fill_between(x_fit, mean_curve_abs - sd_curve_abs, mean_curve_abs + sd_curve_abs, color='#d62728', alpha=0.15, label="Standardabweichung (±1 SD)")
                ...
                ax2.fill_between(x_fit_norm_common, mean_curve_norm - sd_curve_norm, mean_curve_norm + sd_curve_norm, color='#2ca02c', alpha=0.15, label="Standardabweichung (±1 SD)")
